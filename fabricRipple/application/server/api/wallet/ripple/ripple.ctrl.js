...
const index = function(req, res) {
    ...
    /* 조회 결과값 */
    console.log('GET /wallet/ripple?account=%s 응답 메시지', account);
...
const transfer = function(req, res) {
    ...
    /* 송금 결과값 */
    console.log('GET /wallet/ripple/transfer?account=%s&secret=%s&amount=%s&destination=%s 응답 메시지', account, secret, amount, destination);
    console.log(response.getBody().toString());
    return res.send(response.getBody().toString());
};
